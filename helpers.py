...
def fweather(geo):
   ...
   # rez=requests.get('http://www.myweather2.com/developer/forecast.ashx', params={'uac':uac, 'query':geo, 'temp_unit':'c', 'ws_unit': 'kph'})
    ...
    fweather.cache['wurl'] ='https://www.wunderground.com/cgi-bin/findweather/getForecast?query=' + geo
   ...
   # fweather.cache['wtype1'] = wtype1.childNodes[0].nodeValue
    ...
    rez=requests.get('http://api.openweathermap.org/data/2.5/forecast', params={'lat':latlng[0],'lon':latlng[1], 'units':'metric', 'appid': owm_app_id})
    #parse forecast
    data = rez.json()
